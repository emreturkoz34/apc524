...
def read_input(inputname, inputsarray, minargs = 1):
    flag = 0
    for kk in range(len(inputsarray)):
        if inputsarray[kk][0] == inputname:
            ...
            flag = 1
    if flag == 0:
        raise IOError("input %s not specified in input file, syntax is <%s\t file1\t file2\t ...>" % (inputname, inputname))
    return readinput
